...
Declare Function FindWindow Lib "User32" Alias "FindWindowA" (ByVal p1$, ByVal p2&) As Integer
...
Dim X1, X2 As Long
  Select Case AOLCheck()
    ...
    X2 = SendMessageByString(View, 13, X1, S)                  ' S now contains the line of chat
    Case 444:
    'Frame25 = FindWindow("AOL Frame25", 0&)
    'MDIClient = FindChildByClass(Frame25, "MDIClient")
    'Parent = FindChildByClass(MDIClient, "AOL Child")
  ...
  ElseIf AOLCheck() = 32 Then
    'S2 = StripHTML(S2)
      ...
      Do
        ...
        Z = SendMessageByString(X, 12, Len(Ns), Ns)       ' Writes the stuff to be sent to the chat line
