...
Begin VB.MDIForm fChUBMain 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Chibot Ultra Battle 2k23 VirtuaSpace Edition"
   ClientHeight    =   3270
   ClientLeft      =   165
   ClientTop       =   855
   ClientWidth     =   7620
...
'MsgBox frmOptions.cRandArena.Value

CurArena = Rand(1, MaxArena)
'MsgBox Arena(CurArena).name
Dim x
    For x = 1 To 10
      If Arena(CurArena).Desc(x) <> "" Then MsgBox "[ " + Arena(CurArena).Desc(x)
    Next x
...
Dim OK As Integer
  ...
  End Select
  If (GlobalTerm = 0) Then OK = True ' (kDlgBoxfn(Msg, 36, "Quit ChUB?") = 6)
...
Dim x%, PO%
  ...
  tiOutput.Enabled = 0
  ToChUBBot ("CHUBQUIT")
  ScrollSend1 ("{icon:progtag} **It's all over. ChUB Resurrection has been unloaded by " + YourSN + ". " + ccBold + ccUnderline + ccReverse + "**| http://virtua.chat")
  ...
  If Battle = 0 Then
    ...
    Else
      ...
      For x = 1 To MaxPlayers
        InitMoves (x)
      Next x
      ScrollSend ("ChUB Resurrection 2k23 version *" + VerID + "* (Battle #" + TrimStr(GameNum) + ")")
...
Private Sub mnuRunes_Click()
  If Selection = 1 Then
    Send ("<br>**== Choose a Rune! ==** (.randrune for a RANDOM RUNE)")
...
Private Sub RooTimer_Timer()
Debug.Print "tiauto="
Debug.Print tiAuto.interval

End Sub

Private Sub stdin_Timer()
    Dim chattext As String
    Dim chatarray() As String
    Dim lastline As Integer, i As Integer
    
    chattext = ReadInput()
    chatarray() = Split(chattext, vbCrLf)
    'ReDim Preserve chatarray(UBound(chatarray) - 2)
    
    
        i = 0
    For i = LBound(chatarray) To UBound(chatarray) - 1
        
        If chatarray(i) <> "" Then Debug.Print "[" + chatarray(i) + "]"
        
        'Debug.Print test
        Dim S
        S = chatarray(i)
        'just need to now split the chat part from the "SN:  " part:
        S = Split(S, ":  ")
        
        Dim cMsg
        If UBound(S) > 0 Then
            cMsg = S(1)
            If Left$(cMsg, 1) = "." Then cMsg = "/" + Right$(cMsg, Len(cMsg) - 1)
            Debug.Print "{" + S(0) + ":  " + cMsg + "}"
            GetRay(GetSave) = S(0) + ":  " + cMsg 'interesting -Roo
    ...
    'ReDim Preserve chatarray(i-1)
    '### now need to write it back to file or whatever
        
        'Roo

        Dim rResult As String
        Call FileOverwrite(stdin, "")
...
Private Sub tiAuto_Timer()
  Select Case AutoState
    Case 1: ' Load Dataset
      InitFromDisk ("default.ini") '("Simple.ini") '("arcade.ini")
    ...
    Case 2: ' Open Entries
      ...
      mnuRunes_Click
      mnuWeapons_Click
      Send "<br><br><~> **You have one minute to select a character, weapon, and rune!** To see a character's moves, type **.moves-[character]**"
      tiAuto.interval = 60000
      AutoState = 3
    Case 3: ' seems to just be a state of delay
      AutoState = 4
    Case 4:
      AutoState = 5
...
' think it was this timer that scanned the AOL chat for input
' -Roo

  DoEvents
  XX$ = GetLineOfChat() ' "Roo: .aeris" '
  If Not XX$ = "" Then Debug.Print XX$
  'XX is most recent line of chat

  If XX = "" Then Exit Sub
  If Last$ = "" Then
    Last$ = XX$: Exit Sub
  End If
  If (XX$ = Last$) Or Right$(XX, 1) = Chr$(9) Then Exit Sub ' no new line
...
Worked:
  ...
  P4$ = Str$(LoadedX)
  SaveSetting "ChUB Resurrection", "Settings", P2$, P4$
  ScrollSend ("{icon:progtag} **ChUB Resurrection 2k23 " + VerID + " loaded by " + YourSN + " [" + TrimStr(LoadedX) + "x]**")
  ...
  Send ccBold + ccColor + "A total of " + P4$ + " people have died in this game. Rest in pieces.<br>**=== LOADING... ===**<br>"
  Me.Show
...
DoAgain:
  ...
  '  fAFK.Msgs.AddItem (M$)
  'End If
  If SN = "{u=478651906026110976}" And Left$(Ct, 1) = "~" Then HostCommands Ct, Dn%
  If Dn Then Exit Sub
...
If Selection And Not Battle Then
  ...
  If (SNPlayer(SN) <> 0) Then
    DoEvents
    x = SNPlayer(SN)
    If (Skip(Ct, 4) = "OOPS") Or (Skip(Ct, 6) = "REMOVE") Then 'CLEAR CHARACTER
      If (x <> 0) Then
  ...
  For x = 1 To HighSenshi
    ...
    If (UCase(Trim(StripHiAscii(Ct)))) = ("/" + UCase(Senshi(x).PickMe)) And (Senshi(x).PickMe <> "") Then
      OK = True
      X3 = SNPlayer(SN)
      If X3 <> 0 Then 'character already chosen:
...
Dim x4%
  ...
  'If (Auto) Then mnuExit_Click 'Roo
End Sub
...
Dim Hole%
  ...
  tName(Asc("B")) = ccColor + "Blue Team"
  tName(Asc("R")) = ccColor + "Red Team"
  Config.Defect = 0
  TLimit = 0
  'y(1).Enabled = False
  'y(2).Enabled = False
...
Dim Rekkid As Variant
  ...
  If Skip(Ct, 8) = "COMMANDS" Then
    Send ("Host commands: ~EXIT  ~PAUSE  ~SELECT ~CPU ~VOTE  ~CLEAR  ~OPTIONS  ~AVAILABLE  ~RUNES  ~WEAPONS  ~BEGIN  ~END  ~TEAMS  ~CHARS  ~MOVES  ~VOID")
  ...
  End If
  If Skip(Ct, 3) = "CPU" Then
  Dim i
  For i = LBound(P) + 1 To UBound(P)
'    Send "next: " + Str(i) + "/" + Str(P(i).CharID)
    If P(i).CharID = 0 Then
      P(i).CharID = Rand(1, HighSenshi)
      P(i).CPU = 1
      P(i).ScrNam = Senshi(P(i).CharID).FullName
      P(i).TeamID = Chr$(i + 64)
      P(i).Arrogance = 1
      P(i).Wrath = Rand(0, 100)
      P(i).Goodwill = 100
      P(i).Greed = Rand(25, 100)
      'UpdateWin
      Dim newCPU As String
      newCPU = P(i).ScrNam
      Exit For
    End If
  ...
  Next i
    ...
    Send newCPU + " {CPU} joins the match!"
  End If
  If Skip(Ct, 4) = "RANT" Then
    Send ccBold + ccUnderline + ccColor + "~~~--->>> " + UCase(Right$(Ct, Len(Ct) - 6)) + " <<<---~~~"
  End If
